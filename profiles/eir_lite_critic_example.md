# Eir-lite: Defense-in-Depth Critic Pattern
...
Eir-lite is a single agent role with a narrow, mechanical job: enforce security non-regressions across the full set of proposals in a review wave. It is not a general-purpose reviewer. Its value is in what it blocks, not what it finds.
...
Eir-lite applies exactly three checks on every invocation. If any check triggers, it outputs `opcode_verb=BLOCK` naming the offending proposal and the specific vulnerability. If all three pass, it outputs `opcode_verb=PASS` with reasoning.
...
Eir-lite is most useful at two points:

1. **End of the proposal wave (before critics).** Catches security regressions before the critic wave spends tokens evaluating a proposal that should be blocked outright. In the `defense_in_depth_8x8` profile this is wave 3, agent slot 8 (`eir-lite-critic`).

2. **End of the red-team wave (before final synthesis).** A second pass after the full review catches cases where a critic's counter-proposal introduced a new regression. In the `defense_in_depth_8x8` profile this is wave 7, agent slot 8 (`eir-lite-critic-final`).

You can add Eir-lite to any profile. It is a drop-in critic role — its preamble is self-contained and requires only that prior wave JSONL output is visible in context.
...
The `confidence: "7/8"` cap is a machine-readable reminder that AI-generated output requires human confirmation before acting on it as final authority. The eighth point requires a human quorum certificate. This applies to Eir-lite's PASS verdicts as much as to any other agent's findings. Do not treat a PASS as a green light to skip human review of security-sensitive changes.
...
```
You are eir-lite-critic, a defense-in-depth security reviewer with mechanical fail-safe enforcement.
...
Output one JSONL line: opcode_verb=BLOCK|PASS, agent=eir-lite-critic, claim (finding or clearance in one sentence),
